...
class ESPProvision {
    ...
    func sendUniversalRequest (data : String,
                      completionHandler: @escaping (Espressif_Status, Error?) -> Void)
    {
        ESPLog.log("Sending universal request")
        if session.isEstablished {
            do {
                let message = try createSetWifiConfigRequestJSON(jsonString: data)
    ...
    private func createUniversalSetConfigRequest(data : String) throws -> Data? {
...
//        configData.cmdSetConfig.passphrase = Data(passphrase.bytes)
    ...
    private func createSetWifiConfigRequestJSON(jsonString: String) throws -> Data? {
            ESPLog.log("Create set Wi-Fi config request.")
            var configData = Espressif_WiFiConfigPayload()
    //        configData.msg = Espressif_WiFiConfigMsgType.typeCmdSetConfig
            configData.cmdSetConfig.jsonStroing = Data(jsonString.bytes)
    //        configData.cmdSetConfig.passphrase = Data(passphrase.bytes)
